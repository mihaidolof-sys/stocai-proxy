...
function detectChannel(o) {
  ...
  if (mid && MARKETPLACE_NAMES[mid]) return MARKETPLACE_NAMES[mid];
  // website setat = magazin propriu
  if (o.website) return typeof o.website === 'string' ? o.website : 'Magazin propriu';
...
app.get('/inspect', async (req, res) => {
  try {
    ...
    for (const ord of orders) {
      const ch = ord.marketplace_id || ord.marketplace || ord.website || ord.channel || 'necunoscut';
      channels[JSON.stringify(ch)] = (channels[JSON.stringify(ch)] || 0) + 1;
    }
    res.json({ sample, channels_gasite: channels });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
